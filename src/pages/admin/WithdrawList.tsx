...
import type { Withdraw } from "@/services/withdraw.service";
...
const WithdrawList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWithdraws = async () => {
      setIsLoading(true);
      try {
        const data = await WithdrawService.getWithdraws({ page, limit: 10 });
    ...
    <div className="flex flex-col space-y-8 animate-in fade-in duration-700">
      <div className="bg-[#13213F]/40 border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
        <div className="px-8 py-6 border-b border-white/5">
          <h1 className="text-2xl font-bold text-white">List of Withdraw</h1>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/5 text-xs uppercase tracking-wider text-gray-500">
                ...
                <th className="px-8 py-6 font-semibold">Withdraw ID</th>
